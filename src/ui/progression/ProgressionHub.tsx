...
const TABS = ['glory','trees','achievements','challenges'] as const;
type Tab=(typeof TABS)[number];

export function ProgressionHub(){const [tab,setTab]=useState<Tab>('glory');const back=useHudStore((s)=>s.openModeMenu);const language=useProgressionStore((s)=>s.language);const setLanguage=useProgressionStore((s)=>s.setLanguage);
...
{tab==='glory'?<GloryRoadPanel/>:tab==='trees'?<FighterTreePanel/>:tab==='achievements'?<AchievementsPanel/>:<ChallengesPanel/>}
