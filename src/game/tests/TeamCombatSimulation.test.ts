...
describe('team combat simulation', () => {
  ...
  it('creates a hittable helper and permits only one assist extension per combo', () => {
    const simulation = activeSimulation();
    const prepared = simulation.getSnapshot();
    ...
    simulation.step({
      ONLINE_PLAYER_1: actionFrame('ASSIST'),
    ...
    const partner = snapshot.teamBattle.teams.player1.members[1];
