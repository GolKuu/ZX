...
export type FighterRigStance = 'compact' | 'open';
...
export function resetFighterRig(
  rig: FighterRig,
  stance: FighterRigStance,
  time: number,
): void {
  ...
  setRotation(rig.leftArm, 0.04, 0, stance === 'compact' ? -0.68 : 0.26);
  setRotation(rig.rightArm, -0.04, 0, stance === 'compact' ? 0.68 : -0.26);
  setRotation(rig.leftSword, 0, 0, stance === 'compact' ? -0.76 : -0.18);
  setRotation(rig.rightSword, 0, 0, stance === 'compact' ? 0.76 : 0.18);
  setRotation(rig.mouthSword, 0, 0, Math.PI / 2);
  rig.leftSword.visible = stance === 'open';
  rig.mouthSword.visible = stance === 'open';
  rig.rightSword.visible = true;
  hideEffects(rig);
}

export function hideEffects(rig: FighterRig): void {
